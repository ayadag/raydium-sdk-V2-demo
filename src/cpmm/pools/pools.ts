...
async function get() {
  ...
  const poolsM: any[] = [];
      // const pools = await getProgramAccounts6(connection, 1, 2);
  const pools = await getProgramAccounts6(connection, programId);
  for (let index = 0; index < pools.length; index++) {
        await fetchRpcPoolInfo(String(pools[index])).then(res => poolsM.push(res))
  }
  console.log('poolsM: ', poolsM)
...
async function getProgramAccounts6(connection: Connection, programId: PublicKey) {
  ...
  console.log('paginatedKeys: ', paginatedKeys)
